...
| Proxy listener control | `listener status`, `listener stop`, `listener start`, `listener update` | [proxy listener control](proxy-listener-control.md) |
...
When a change touches one row, read that feature file and cover every affected entry point. The bundled helper proves service lifecycle, one HTTP capture, filtered listing, and detail inspection. It does not cover every listener transition, HTTPS interception, or pagination.
